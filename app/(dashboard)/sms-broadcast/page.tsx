...
export default function SmsBroadcastPage() {
  ...
  const handleAddContact = async () => {
    ...
    // Format Philippines number automatically
    ...
    try {
      ...
      alerts.toastSuccess("Customer added successfully!");
      setShowAddContact(false);
      setNewContactName('');
      setNewContactPhone('');
    } catch (err: any) {
      alerts.error("Failed to add customer", err?.data?.message || err?.message);
    }
  };
  ...
  const handleDeleteContact = async (id: string, name: string) => {
    ...
    if (confirmed) {
      try {
        ...
        alerts.toastSuccess("Customer deleted successfully!");
        setSelectedContacts(prev => prev.filter(cId => cId !== id));
      } catch (err: any) {
        alerts.error("Failed to delete customer", err?.data?.message || err?.message);
      }
    }
  };
  ...
  const handleUpdateContact = async () => {
    ...
    try {
      ...
      alerts.toastSuccess("Customer updated successfully!");
      setShowEditContact(false);
      setEditingContactId(null);
    } catch (err: any) {
      alerts.error("Failed to update customer", err?.data?.message || err?.message);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      <PageHeader 
        title="SMS Broadcast" 
        subtitle="Send personalized text messages to your customers and leads."
      />
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-center max-w-2xl mx-auto py-4">
            <div className="flex items-center gap-3">
              ...
              <span className={cn("font-bold text-sm transition-colors", step >= 1 ? "text-primary" : "text-text-muted")}>Compose Message</span>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className={cn("font-semibold text-sm transition-colors", step >= 2 ? "text-primary" : "text-text-muted")}>Select Contacts</span>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-xl border border-border p-6 shadow-sm space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={cn("text-sm font-semibold", message.length > 160 ? "text-red-500" : "text-emerald-500")}>
            ...
            <div className="space-y-6">
              ...
              {isCustomersLoading ? (
                ...
                <div className="bg-[#fafafa] border border-dashed border-border/60 rounded-xl p-12 text-center flex flex-col items-center justify-center min-h-[300px]">
                  ...
                  <h3 className="font-bold text-lg text-black mb-2">No contacts yet</h3>
                  <p className="text-sm text-text-muted max-w-sm">
                    Add them manually or import from a file to start building your SMS broadcast list.
                  </p>
                </div>
              ) : (
              ...
              <div className="flex items-center justify-between pt-4 border-t border-border mt-8">
                <button 
                  onClick={() => setStep(1)}
                  className="text-sm font-semibold text-black hover:text-primary transition-colors flex items-center gap-1"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-bold text-lg text-black">Add New Contact</h3>
            ...
            <div className="p-4 border-t border-border flex justify-end gap-3 bg-gray-50/50">
              ...
              <Button variant="primary" onClick={handleAddContact} disabled={isCreating} className="font-bold px-6 shadow-sm">
                {isCreating ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : "Add Contact"}
              </Button>
            </div>
          </div>
        </div>
      ...
      {showEditContact && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-bold text-lg text-black">Edit Contact</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-5 border-b border-border shrink-0">
              <h3 className="font-bold text-xl text-black">Import Contacts</h3>
              ...
              {importTab === 'manual' && (
                <div className="space-y-6">
                  <div className="space-y-1.5 max-w-sm">
                    <label className="text-sm font-medium text-text-muted">Group for all these contacts (Optional)</label>
                  ...
                  <Button variant="primary" className="w-full py-3 font-bold rounded-xl shadow-sm mt-4">
                    Save All Contacts
                  </Button>
                </div>
              )}
