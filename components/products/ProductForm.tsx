...
//         alerts.error(
//           "File Too Large",
//           `Image "${file.name}" exceeds the maximum limit of 5MB.`,
//         );
//         return false;
//       }
//       return true;
...
//       if (key === "images" || key === "features[]" || key === "features")
//         return;
//       payload[key] = value;
//     });
...
//           {/* LEFT COLUMN (full width — no more right column) */}
...
//                   name="origin"
//                   required
//                   defaultValue={initialData?.origin ?? "China"}
...
//                   name="brandName"
//                   required
//                   defaultValue={initialData?.brandName ?? "Komatsu"}
...
//                     >
//                       <Plus size={16} className="mr-2" /> Add Another Feature
//                     </Button>
//                   </div>
//                 </div>
//                 <div className="space-y-1.5">
...
//                 <h3 className="font-bold text-lg text-text-primary">Images</h3>
...
//                       <Plus size={16} /> Add Images from Computer
//                       <input
//                         ref={fileInputRef}
//                         type="file"
//                         accept="image/*"
...
//                     The first image will be used as the featured image. Maximum
//                     5 images. Highest 5MB per image.
//                   </p>
//                 </div>
//               </CardBody>
//             </Card>
...
export const ProductForm = ({ initialData }: ProductFormProps) => {
  ...
  const handleSubmit = async (
    ...
    // Build a proper multipart FormData (matches backend's multer.array("images", 5))
    // instead of a JSON payload with base64 strings — base64 in JSON was blowing past
    // express.json()'s default 100kb body limit long before hitting the 5MB per-file check.
    ...
    features
      .filter((f) => f.trim() !== "")
      .forEach((f) => submitData.append("features", f));

    submitData.set("status", isDraft ? "draft" : "active");
    submitData.set("categoryId", categoryId);
    submitData.set("condition", condition);

    // New locally-picked files go under "images" (matches backend's multer.array("images", 5)).
    // Note: without a backend change, uploading NEW images on an edit will replace all images —
    // there's no way to keep old ones + add new ones at the same time from the frontend alone.
