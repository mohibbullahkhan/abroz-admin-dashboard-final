...
import { Eye, EyeOff, Mail, Lock, ArrowLeft, CheckCircle, ShieldCheck, KeyRound } from 'lucide-react';
...
// ─── Types ────────────────────────────────────────────────────────────────────
type AuthView = 'login' | 'forgot-email' | 'forgot-verify' | 'forgot-reset' | 'forgot-success';
    ...
    <div className="hidden lg:flex w-1/2 relative flex-col justify-center items-center p-12 bg-gray-50 border-r border-border overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center text-center justify-center space-y-12">
        <div className="flex flex-col items-center">
          <Image src="/Logo.png" alt="Abroz Parts+ Logo" width={240} height={70} className="w-auto h-16 object-contain" />
        </div>
        <div className="space-y-6 flex flex-col items-center">
          ...
          <p className="text-lg text-text-muted max-w-md mx-auto">
            The ultimate admin portal for the Philippines' leading machinery parts provider.
          </p>
        </div>
      </div>
    </div>
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.status === 'FETCH_ERROR') {
        errorMsg = 'Network error. Please check your connection or CORS settings.';
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alerts.success('Password Reset Successful', 'You can now sign in with your new password.');
  ...
  const renderLogin = () => (
    <div className="w-full max-w-md space-y-8">
      <div className="lg:hidden flex flex-col items-center mb-8 text-center">
        <Image src="/Logo.png" alt="Abroz Parts+ Logo" width={200} height={60} className="w-auto h-12 object-contain mb-4" />
      </div>
      <div className="text-left">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Welcome Back</h2>
        <p className="text-text-muted">
          Please enter your credentials to access the <span className="text-gray-700 font-medium">admin portal</span>.
        </p>
      </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="relative">
                <Input
                  ...
                  icon={<Lock size={18} />}
                  suffix={
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-text-muted hover:text-gray-900 transition-colors">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                <span className="text-sm text-text-muted group-hover:text-gray-900 transition-colors">Remember me</span>
              </label>
              <button type="button" onClick={() => setView('forgot-email')} className="text-sm text-primary hover:text-primary-dark font-medium transition-colors">
                Forgot password?
              </button>
            </div>

            <Button type="submit" variant="primary" className="w-full py-4 text-base font-bold uppercase tracking-widest shadow-[0_0_20px_rgba(245,158,11,0.2)]" loading={isLoginLoading}>
  ...
  const renderForgotEmail = () => (
    <div className="w-full max-w-md space-y-8">
      <button onClick={resetFpFlow} className="flex items-center gap-2 text-sm text-text-muted hover:text-gray-900 transition-colors group">
        ...
        {['Send Email', 'Verify Code', 'Reset Password'].map((label, i) => (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[10px] font-medium ${i === 0 ? 'text-primary' : 'text-gray-400'}`}>{label}</span>
            </div>
            {i < 2 && <div className={`flex-1 h-0.5 rounded mb-4 ${i === 0 ? 'bg-gray-200' : 'bg-gray-100'}`} />}
          </React.Fragment>
        ))}
      </div>
  ...
  const renderForgotVerify = () => (
    <div className="w-full max-w-md space-y-8">
      <button onClick={() => setView('forgot-email')} className="flex items-center gap-2 text-sm text-text-muted hover:text-gray-900 transition-colors group">
        <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
        Change email
      </button>
      ...
      <div>
        ...
        <p className="text-text-muted text-sm leading-relaxed">
          ...
          <span className="font-semibold text-gray-800">{fpEmail || 'your email'}</span>.<br />
        ...
        {['Send Email', 'Verify Code', 'Reset Password'].map((label, i) => (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[10px] font-medium ${i <= 1 ? 'text-primary' : 'text-gray-400'}`}>{label}</span>
            </div>
            {i < 2 && <div className={`flex-1 h-0.5 rounded mb-4 ${i === 0 ? 'bg-primary' : 'bg-gray-100'}`} />}
          </React.Fragment>
        ))}
      </div>
        ...
        <CardBody className="p-8">
          <form onSubmit={handleVerifyOtp} className="space-y-6">
            <div className="space-y-3">
              <label className="text-sm font-medium text-text-muted block text-center">Enter 6-Digit Code</label>
            ...
            <div className="text-center text-sm text-text-muted">
              ...
              {resendCooldown > 0 ? (
                <span className="text-gray-400">Resend in <span className="font-bold text-primary">{resendCooldown}s</span></span>
              ) : (
                <button type="button" onClick={handleResend} className="text-primary hover:text-primary-dark font-semibold transition-colors">
                  Resend Code
                </button>
              )}
            </div>
    ...
    return (
      <div className="w-full max-w-md space-y-8">
        <div>
          ...
          <p className="text-text-muted text-sm leading-relaxed">
            Create a strong new password for your account. Make sure it's at least 8 characters.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {['Send Email', 'Verify Code', 'Reset Password'].map((label, i) => (
            <React.Fragment key={label}>
              ...
              {i < 2 && <div className="flex-1 h-0.5 rounded mb-4 bg-primary" />}
            </React.Fragment>
          ))}
        </div>
              ...
              <div className="space-y-4">
                <div className="relative">
                  <Input
                    ...
                    icon={<Lock size={18} />}
                    suffix={
                      <button type="button" onClick={() => setShowNew(!showNew)} className="text-text-muted hover:text-gray-900 transition-colors">
                ...
                <div className="relative">
                  <Input
                    ...
                    icon={<Lock size={18} />}
                    suffix={
                      <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="text-text-muted hover:text-gray-900 transition-colors">
